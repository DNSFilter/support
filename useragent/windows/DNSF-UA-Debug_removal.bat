@ECHO OFF
REM Version 0.1
REM Created by Rick Cohen on 11JAN2022
REM Modified from DNSF_UA_debug.bat at https://github.com/DNSFilter/support
...
@echo off 
echo Uninstalling DNSFilter Agent if it exists.
wmic product where "name='DNSFilter Agent'" call uninstall
echo Uninstalling DNS Agent if it exists.
wmic product where "name='DNS Agent'" call uninstall
echo DNSFilter software Removed!
echo Removing Registry Keys and Folders that are potentially left behind.
@echo off
reg delete "HKEY_LOCAL_MACHINE\SOFTWARE\DNSAgent" /f
reg delete "HKEY_LOCAL_MACHINE\SOFTWARE\DNSFilter" /f
if exist "C:\Program Files\DNSFilter Agent" rmdir /s /q "C:\Program Files\DNSFilter Agent"
if exist "C:\Program Files\DNSAgent" rmdir /s /q "C:\Program Files\DNS Agent"

echo Logs are collected in C:\temp and software has been fully removed.
PAUSE
